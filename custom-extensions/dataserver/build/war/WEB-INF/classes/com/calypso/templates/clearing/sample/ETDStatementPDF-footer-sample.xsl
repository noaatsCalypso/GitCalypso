...
<xsl:stylesheet
  ...
  <xsl:attribute-set name="footer-block-style">
    ...
    <xsl:attribute name="color">#1A237E</xsl:attribute>
  ...
  <xsl:template name="footerContent">
    ...
    <fo:static-content
      flow-name="footer">
      <!-- Customized the page count to align right, and added top border to block -->
      <fo:block
        border-top="0.5px solid #1A237E"
        text-align="right"
        xsl:use-attribute-sets="footer-block-style">
        <fo:page-number />
        of
