<xsl:stylesheet xmlns:stmt="urn:com:calypso:clearing:statement:etd"
	...
	<xsl:template match="stmt:row" mode="openPositionsRow">
		...
		<tr class="{$trClass}">
			...
			<td class="ALIGN_RIGHT">
				<xsl:value-of
					select="stmtext:formatNumberWithQuoteType(@close, number(@productContractDecimals), @productQuoteType, $useExtendedFuture32)" />
			</td>
			<td class="CURRENCY">
				<xsl:value-of select="@currency" />
			</td>
