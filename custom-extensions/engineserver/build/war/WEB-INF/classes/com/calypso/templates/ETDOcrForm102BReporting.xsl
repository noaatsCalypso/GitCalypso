...
<xsl:stylesheet version="1.0" xmlns:ltr="urn:com:calypso:clearing:cftc:ltr" xmlns:cftcForm102="urn:com:calypso:clearing:cftc:ocr" xmlns:cftc="urn:com:calypso:clearing:cftc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/ltr:LTReport">
		<cftcBatch>
			...
			<xsl:for-each select="/ltr:LTReport/ltr:account[generate-id() = generate-id(key('cftcAccountNumberKey',@cftcAccountNumber)[1])]">
				...
				<xsl:call-template name="buildCftcForm102B">
	...
	<xsl:template name="buildCftcForm102B">
		<xsl:param name="cftcAccount"/>
  			<xsl:if test="/ltr:LTReport/ltr:account[@cftcAccountNumber=$cftcAccount]/cftcForm102:accountDetails/@ocrVTThresholdBreached = 'true' and /ltr:LTReport/ltr:account[@cftcAccountNumber=$cftcAccount]/cftcForm102:accountDetails/@totalQuantityForTheDay != 0" >
				<cftcForm102>
					<xsl:attribute name="accountCategory"><xsl:value-of select="/ltr:LTReport/ltr:account[@cftcAccountNumber=$cftcAccount]/cftcForm102:accountDetails/@vtAccountCategory"/></xsl:attribute>
					<xsl:attribute name="accountNumber"><xsl:value-of select="$cftcAccount"/></xsl:attribute>
					<xsl:attribute name="accountClass"><xsl:value-of select="/ltr:LTReport/ltr:account[@cftcAccountNumber=$cftcAccount]/cftcForm102:accountDetails/@accountClass"/></xsl:attribute>
					<!-- 
					<xsl:attribute name="ocrVTThresholdBreached"><xsl:value-of select="/ltr:LTReport/ltr:account[@cftcAccountNumber=$cftcAccount]/cftcForm102:accountDetails/@ocrVTThresholdBreached"/></xsl:attribute>
					 -->
	...
	<xsl:template name="buildRelatedAccountInfo">
		...
		<xsl:if test="$positionNode/@isTradedToday = 'true'">
			<relatedAcctInfo>
