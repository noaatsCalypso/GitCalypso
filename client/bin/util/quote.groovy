...
def save_quote(pricingEnv, valueDate) {

		md =  ds.getRemoteMarketData()
		pricingEnv = md.getPricingEnv("default" ,new JDatetime())
		qs = pricingEnv.getQuoteSet();
		
		save_one_quote(md, qs, JDate.getNow(), "FX.EUR.USD", 1.2)
		save_one_quote(md, qs, JDate.getNow(), "FX.GBP.USD", 1.8)
		save_one_quote(md, qs, JDate.getNow(), "FX.USD.JPY", 111.0)
