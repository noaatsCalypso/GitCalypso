IF NOT EXISTS (select id from cm_simm_regulator)
BEGIN
...
IF NOT EXISTS (select id from cm_simm_account)
BEGIN
  ...
  EXECUTE 'UPDATE cm_account SET csadetails_id = lower(newid(1))'
...
IF NOT EXISTS (select id from cm_flat_simm_param)
BEGIN
    ...
    EXECUTE('INSERT INTO cm_flat_simm_param (id, tenant_id, version, creation_date, creation_user,
      creation_user_type, last_update_date, last_update_user, last_update_user_type, regulator,
      accountdetails_id, direction, parameter_type, value, criteria)
      (SELECT lower(newid(1)) id, s.tenant_id, s.version,
    ...
    EXECUTE('INSERT INTO cm_flat_simm_param (id, tenant_id, version, creation_date, creation_user,
      creation_user_type, last_update_date, last_update_user, last_update_user_type, regulator,
      accountdetails_id, direction, parameter_type, value, criteria)
      (SELECT lower(newid(1)), p.tenant_id,
