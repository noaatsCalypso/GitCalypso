begin
add_column_if_not_exists ('eto_contract','last_delivery_date','timestamp null');
end;
/
begin
add_column_if_not_exists ('eto_contract','last_delivery_rule','number');
end;
/
begin
add_column_if_not_exists ('eto_contract','dateformat','number');
end;
/
begin
add_column_if_not_exists ('eto_contract','first_delivery_rule','number');
end;
/
begin
add_column_if_not_exists ('product_eto','last_delivery_date','timestamp null');
end;
/
begin
add_column_if_not_exists ('product_eto','first_delivery_date','timestamp null');
end;
/ 
...
using
	(select
		entity_attributes.entity_id as id, 
		entity_attributes.attr_value as attr_value
	from
		entity_attributes,
		listed_contract
	where
		...
		and
		entity_attributes.attr_name='ContractLongName') result
on
	(listed_contract.contract_id = result.id)	
when matched then
	update set
		listed_contract.long_name=result.attr_value
;
...
using
	...
	where
		...
		entity_attributes.entity_type='ETOContract')
		and
		entity_attributes.attr_name='ClearingExchangeTicker') result
on
	(listed_contract.contract_id = result.id)	
when matched then
	update set
		listed_contract.exch_clrg_ticker=result.attr_value
;
...
using
	...
	where
		...
		entity_attributes.entity_type='ETOContract')
		and
		entity_attributes.attr_name='PremiumPaymentConvention') result
on
	(listed_contract.contract_id = result.id)	
when matched then
	update set
		listed_contract.prem_pmt_conv=result.attr_value
;
...
using
	(select unique
		le.short_name as short_name,
		lea.attribute_value as attr_value
	from
		le_attribute lea,
		listed_contract lc,
		legal_entity le
	where
		lea.legal_entity_id = le.legal_entity_id
		and
		le.short_name = lc.exchange_code
		and
		lea.attribute_type='MIC') result
on
	(listed_contract.exchange_code = result.short_name)	
when matched then
	update set
		listed_contract.exchange_mic=result.attr_value
;
...
using
	(select distinct
		entity_attributes.entity_id as id, 
		entity_attributes.attr_value as attr_value,
		entity_attributes.entity_type as entity_type
	from
		entity_attributes,
		listed_contract
	where
		...
		entity_attributes.entity_type='ETOContract')
		and
		entity_attributes.attr_name='Quote Decimals') result
on
	(listed_contract.contract_id = result.id and listed_contract.contract_type = result.entity_type)	
when matched then
	update set
		listed_contract.quote_decimals=result.attr_value
;
...
using
	(select unique
		listed_contract.contract_id as id,
		future_contract.quote_decimals as qd
	from
		future_contract,
		listed_contract
	where
		future_contract.contract_id = listed_contract.contract_id) result
on
	(listed_contract.contract_id = result.id)	
when matched then
	update set
		listed_contract.quote_decimals=result.qd
;
