...
<xsl:stylesheet version="1.0"
  ...
  <!-- FINANCIAL SUMMARY SECTION -->
	...
	<xsl:key name="financialSummaryKey"
		match="stmt:section[@id='financialSummary']/stmt:row" use="@currency" />
    ...
    <!-- pageSize represents the number of currencies per page -->
		...
		<xsl:variable name="numCurrencies"
			select="count(stmt:row[generate-id() = generate-id(key('financialSummaryKey', @currency))])" />
    ...
    <fo:block>
      <xsl:value-of select="concat('NUM CCYS: ', $numNonBaseCurrencies)"></xsl:value-of>
    </fo:block>
    <fo:block>
			...
			<xsl:for-each
				...
				<xsl:if
					...
					<fo:table-cell
						...
						<fo:block xsl:use-attribute-sets="quantity-block-style">
							<xsl:value-of
								select="stmtext:formatNumber(../stmt:row[@currency=$ccy and @id=$idToMatch]/@value, $ccy)" />
						</fo:block>
					</fo:table-cell>
