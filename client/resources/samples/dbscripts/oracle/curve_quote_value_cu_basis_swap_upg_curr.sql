...
/* e.g Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg Becomes Swap.USD.1Y.LIBOR.3M/BMA.1W.Bloomberg */
...
CREATE TABLE tmp_crv_qt_val
	...
	tmp_source2 VARCHAR(50) NULL,		
	fixed_cpn_freq VARCHAR(255) null,	
	CONSTRAINT pk_tmp_crv_quote_value PRIMARY KEY 	(curve_id, curve_date, orig_quote_name) USING INDEX TABLESPACE CALYPSOIDX
) 
;
...
/* ************************************************** */
/* tmp_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
...
/* tmp_quote_name = 1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
...
/* tmp_quote_name = 1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1Y */

UPDATE tmp_crv_qt_val SET tmp_maturity_tenorstr=substr(tmp_quote_name,1 ,instr(tmp_quote_name,'.')-1 )
...
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1Y */
/* tmp_currency1 = NULL  bcos it not present in domain values */
...
/* tmp_quote_name = LIBOR.3M/BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1Y */
/* tmp_currency1 = NULL  bcos it not present in domain values */
...
/* tmp_quote_name = TMP.LIBOR.3M/BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1Y */
/* tmp_currency1 = NULL  bcos it not present in domain values */
...
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1Y */
/* tmp_currency1 = TMP  bcos it  present in domain values */
...
/* tmp_quote_name = LIBOR.3M/BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1M */
/* tmp_maturity_tenor = 30 */
/* tmp_currency1 = TMP  bcos it  present in domain values */
...
/* tmp_maturity_tenor = 7 */
/* tmp_currency1 = TMP  bcos it  present in domain values */
/* tmp_rate_index1 = LIBOR  bcos it  present in domain values */
...
/* tmp_quote_name = 3M/BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1M */
/* tmp_maturity_tenor = 7 */
/* tmp_currency1 = TMP  bcos it  present in domain values */
...
/* tmp_quote_name = BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1M */
/* tmp_maturity_tenor = 7 */
/* tmp_currency1 = TMP  bcos it  present in domain values */
...
/* tmp_quote_name = TMP.BMA.1W.Bloomberg */
/* orig_quote_name = Swap.1Y.LIBOR.3M/BMA.1W.Bloomberg */
/* tmp_maturity_tenorstr = 1M */
/* tmp_maturity_tenor = 7 */
/* tmp_currency1 = TMP  bcos it  present in domain values */
...
UPDATE tmp_crv_qt_val set tmp_tenor2=substr(tmp_quote_name,1 ,instr(tmp_quote_name,'.')-1 ) 
WHERE tmp_basis_swap=1  and instr(tmp_quote_name,'.') <> 0 
;


UPDATE tmp_crv_qt_val set tmp_source2=substr(tmp_quote_name,instr(tmp_quote_name,'.')+1,length(tmp_quote_name)-instr(tmp_quote_name,'.')) 
WHERE tmp_basis_swap=1  and instr(tmp_quote_name,'.') <> 0 
;
...
WHERE tmp_currency2 <> 'TMP'  and tmp_basis_swap=1
;
...
/* tmp_rate_index2 = BMA */
/* tmp_tenor2 = 1W   */
/* quote_name = Swap.TMP.1Y.LIBOR.3M/BMA.1W.Bloomberg */
...
tmp_rate_index1 ||'.'||
tmp_tenor1 || '/'  || tmp_rate_index2 || '.' || tmp_tenor2 || '.' || tmp_source2
WHERE tmp_currency2 = 'TMP'
;	
...
UPDATE tmp_crv_qt_val SET new_quote_name=quote_name 
WHERE  tmp_basis_swap=1 and tmp_crv_qt_val.quote_name like 'Swap%'
;
...
AND tmp_basis_swap=1 and tmp_source2 is NULL
;	
...
AND  curve_quote_value.quote_name=tmp_crv_qt_val.orig_quote_name 
AND  tmp_basis_swap=1 
AND  new_quote_name is NOT NULL and tmp_tenor2 is not NULL AND tmp_currency1='TMP')
;
